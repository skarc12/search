...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    "*** YOUR CODE HERE ***"
    self.start = (startingGameState.getPacmanPosition(), False, False, False, False)
  def getStartState(self):
...
def cornersHeuristic(state, problem):
  ...
  for i in range (0,4):
    if visited[i]:
        unvisited.remove(corners[i])

  while len(unvisited) > 0:
    closestDistance = None
    for corner in unvisited:
        if closestDistance == None or util.manhattanDistance(curr, corner) < closestDistance:
            closestDistance = util.manhattanDistance(curr, corner)
            prev = corner
...
def foodHeuristic(state, problem):
  ...
  for i in range(len(allFood)):
      for food in allFood:
          if (util.manhattanDistance(food, position) < util.manhattanDistance(minFood, position)):
              minFood = food
      sortFood.append(minFood)
      allFood.remove(minFood)
      if allFood:
          minFood = allFood[0]
  while (len(sortFood) > 0):
       node = sortFood.pop(0)
       distance += util.manhattanDistance(node, curr)
       curr = node
